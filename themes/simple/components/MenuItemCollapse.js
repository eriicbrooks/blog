  ...
  return <>
        <div className='w-full px-8 py-3 text-left border-b dark:bg-hexo-black-gray dark:border-black' onClick={toggleShow} >
            {!hasSubMenu && <Link
                ...
                <span className='text-blue-400 hover:text-red-400 transition-all items-center duration-100'>{link?.icon && <span className='mr-2'><i className={link.icon}/></span>}{link?.name}</span>
            </Link>}
             {hasSubMenu && <div
                ...
                <span className='text-blue-400 hover:text-red-400 transition-all items-center duration-100'>{link?.icon && <span className='mr-2'><i className={link.icon}/></span>}{link?.name}</span>
               <i className={`px-2 fa fa-plus transition-all duration-100 ${isOpen && 'rotate-45'} text-gray-400`}></i>
            </div>}
        </div>
        ...
        {hasSubMenu && <Collapse isOpen={isOpen} onHeightChange={props.onHeightChange}>
            {link.subMenus.map((sLink, index) => {
              return <div key={index} className='font-extralight dark:bg-black text-left px-10 justify-start text-blue-400 bg-gray-50 hover:bg-gray-50 dark:hover:bg-gray-900 tracking-widest transition-all duration-100 border-b dark:border-gray-800 py-3 pr-6'>
